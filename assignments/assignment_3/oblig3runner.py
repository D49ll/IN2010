...
ALGS1 = [heap.sort, quick.sort, insertion.sort, selection.sort]

# Put the sorting algorithms under test for part 2 here
ALGS2 = [heap.sort, quick.sort, insertion.sort, selection.sort]
...
def run_algs_part1(A, infilename,large_n):
    if large_n:
        part = 2
    else:
        part = len(ALGS1)

    for alg in ALGS1[:part]:
...
def run_algs_part2(A, infilename, large_n):
    if large_n:
        part = 2
    else:
        part = len(ALGS2)
    ...
    header = makeheader(ALGS2[:part], digits)
    ...
    for i in range(0, len(A) + 1, INCREMENT):
        ...
        for alg in ALGS2[:part]:
            row += ', ' + runalg(alg, A, i, discarded)

        if set(ALGS2[:part]) == discarded:
            break
